...
pub async fn handle_pay_with_nevermined(
    ...
    if zpi_proof.trim().is_empty() && !has_supplied_token {
        ...
        return Ok(Json(FarmToolResponse::ok(json!({
            ...
            "instructions": "PREFERRED FLOW: call this tool with verify_only=true + external_id + zpi_proof_id so the merchant resolves attester_proof_id, then call zpi-zkpay's pay-with-nevermined-merchant-settles (it mints the x402 token on the user's behalf without leaking the user's NVM API key to the merchant), then call this tool with token_ref/x402_access_token + plan_id + external_id + zpi_proof_id + attester_proof_id. LEGACY: call chp_save, then zpi_generate_zkp with this external_id and intent_type='spend', then call pay-with-nevermined again with zpi_proof + external_id (this minted token will come from the merchant's own NVM_API_KEY)."
        }))));
    }
    ...
    if req.payment_method == "nevermined_card" {
        ...
        return Ok((
            ...
            Json(FarmToolResponse::ok(json!({
                ...
                "instructions": "This external_id is a UUIDv4. Call chp_save, then zpi_prove_intent with this external_id and intent_type='spend' (save zkp_proof_id). Then: (1) call pay-with-nevermined with verify_only=true + external_id + zpi_proof_id so the merchant verifies the proof and returns attester_proof_id; (2) call zpi-zkpay's pay-with-nevermined-merchant-settles to mint; (3) call pay-with-nevermined with token_ref/x402_access_token + plan_id + external_id + zpi_proof_id + attester_proof_id to settle."
            }))),
        ));
    }
        ...
        json!({
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "zpi_proof": {
                        "type": "string",
                        "description": "ZPI proof blob/string from zpi_generate_zkp. Required for legacy mint flow only."
                    },
                    "zpi_proof_id": {
                        "type": "string",
                        "description": "zkp_proof_id from zpi_generate_zkp. When supplied, the merchant pulls the proof from the attester (GET /proofs/{id}/verify) and rejects the payment if invalid."
                    },
                    "attester_proof_id": {
                        "type": "string",
...
9. To purchase with VGS card flow, call farm-checkout with payment_method='vgs_card'.
    ...
    Run chp_save + zpi_generate_zkp (intent_type='spend'), then call
